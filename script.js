...
const API = 'https://code-complexity-analyzer-lcz1.onrender.com/analyze';
...
function buildOptimizedHTML(el, data) {
  ...
  const code     = versions[activeLangTab] || '// Not available for this language';
...
function renderBottom(data) {
  ...
  const flowchartContent = data.flowchart
    ? `<div class="mermaid">${esc(data.flowchart)}</div>`
